...
pub(super) fn build_snapshot(
    ...
    for result in results {
        let Some(detail) = state
            .database
            .get_flow_detail(&result.flow.id)
            .map_err(storage_error)?
        else {
            continue;
        };
        flows.push(ComparableFlow {
            request_body: body_for_compare(
                detail
                    .request
                    .as_ref()
            ...
            response_body: body_for_compare(
                detail
                    .response
                    .as_ref()
...
pub(super) fn sdk_context_for_detail(
    ...
    Ok(Some(AppContextEvidence {
        ...
        platform: client
            .as_ref()
            .map(|client| platform_name(&client.platform).to_string()),
