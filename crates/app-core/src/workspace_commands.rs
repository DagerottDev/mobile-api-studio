...
pub fn upsert_environment(
    ...
    let id = input
        .id
        .unwrap_or_else(|| format!("environment-{timestamp}"));
    let existing = state
        .database
        .list_environments()
        .map_err(storage_error)?
                    ...
                    let previous_value = secret_store
                        ...
                        .ok_or_else(|| {
                            AppError::new(
                                ...
                                format!(
                                    "Secret variable '{}' has no value in secure storage.",
                                    previous.key
                                ),
                                true,
                            )
        ...
        let value = if variable.is_secret {
            let reference = variable.secret_ref.as_deref().ok_or_else(|| {
                AppError::new(
                    ...
                    format!(
                        "Secret variable '{}' is missing its secure-store reference.",
                        variable.key
                    ),
                    true,
                )
