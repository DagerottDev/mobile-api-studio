...
pub fn sdk_flow_ids_matching(
    ...
    let flows = state.database.list_flows(10_000).map_err(storage_error)?;
                ...
                Ok(envelope) => {
                    if database.record(&envelope).is_ok() {
                        if let (Some(request_id), Some(session_database)) =
                            (envelope.event.request_id(), session_database.as_ref())
                        {
                            if let Ok(Some(client)) =
                                database.get_client(envelope.event.client_id())
                            {
