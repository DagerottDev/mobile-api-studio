...
use mock_core::{
    normalized_path_for_flow, MockBodyEncoding, MockBodyOverride, MockFailureMode,
    MockPathMatch, MockRule, MockRulesDocument, MOCK_SCHEMA_VERSION,
};
...
pub fn create_mock_from_flow(
    ...
    let detail = state
        ...
        .ok_or_else(|| AppError::new("mock_flow_detail_missing", "Full flow details are required to create a mock.", true))?;
...
pub fn set_mock_rule_enabled(
    ...
    let database = mock_database(&state)?;
    database.set_enabled(&id, enabled).map_err(mock_storage_error)?;
    publish_current_rules(&state, &database)
}
...
pub fn set_mock_rule_priority(
    ...
    let database = mock_database(&state)?;
    database.set_priority(&id, priority).map_err(mock_storage_error)?;
    publish_current_rules(&state, &database)
}
...
fn publish_current_rules(state: &State<'_, AppState>, database: &MockDatabase) -> Result<(), AppError> {
    let rules = database.list_rules().map_err(mock_storage_error)?;
...
fn publish_rules(state: &State<'_, AppState>, rules: Vec<MockRule>) -> Result<(), AppError> {
    ...
    let bytes = serde_json::to_vec_pretty(&document).map_err(|error| {
        AppError::new("mock_rules_serialize_failed", error.to_string(), true)
    })?;
    let directory = state.capture_engine.conf_dir();
    fs::create_dir_all(directory).map_err(|error| {
        AppError::new("mock_rules_directory_failed", error.to_string(), true)
    })?;
...
fn validate_rule(rule: &MockRule) -> Result<(), AppError> {
    if rule.name.trim().is_empty() {
        return Err(AppError::new("mock_rule_name_required", "Mock rule name is required.", true));
    }
    if rule.path_pattern.trim().is_empty() {
        return Err(AppError::new("mock_rule_path_required", "Mock path pattern is required.", true));
    }
