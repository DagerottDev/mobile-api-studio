...
pub fn upsert_mock_fixture(
    ...
    fixture_database(&state)?.upsert(&fixture).map_err(fixture_error)?;
    Ok(fixture)
}
...
pub fn create_fixture_from_flow(
    ...
    let response = detail.response.as_ref().ok_or_else(|| AppError::new(
    ...
    let fixture = MockFixture {
        ...
        name: sanitize_name(
            name.as_deref(),
            &format!("{} {}{}", detail.summary.method, detail.summary.host, detail.summary.path),
        ),
...
pub fn apply_fixture_to_mock(
    ...
    let fixture = fixture_database(&state)?
        ...
        .ok_or_else(|| AppError::new("mock_fixture_missing", "Response fixture not found.", true))?;
...
fn validate_fixture(fixture: &MockFixture) -> Result<(), AppError> {
    if fixture.name.trim().is_empty() {
        return Err(AppError::new("mock_fixture_name_required", "Fixture name is required.", true));
    }
