...
pub fn set_ai_settings(
    ...
    if model.is_empty() {
        return Err(AppError::new("ai_model_required", "Choose an OpenAI model.", true));
    }
    let secret_keys = normalize_secret_keys(input.secret_json_keys);
    state.database.set_preference(&AppPreference {
...
pub async fn diagnose_flow_with_ai(
    ...
    let preview = preview_value(&value, &state)?;
    verify_preview(&preview, &input)?;
    run_ai(AiTaskKind::FlowDiagnosis, format!("flow:{flow_id}"), preview, &state).await
}
...
fn load_settings(state: &State<'_, AppState>) -> Result<AiSettingsSnapshot, AppError> {
    let provider = state.database.get_preference(PREF_AI_PROVIDER).map_err(storage_error)?
        .map(|value| value.value)
        .unwrap_or_else(|| OPENAI_PROVIDER_ID.into());
    let model = state.database.get_preference(PREF_AI_MODEL).map_err(storage_error)?
        .map(|value| value.value)
        .unwrap_or_else(|| DEFAULT_OPENAI_MODEL.into());
    let secret_json_keys = state.database.get_preference(PREF_AI_SECRET_KEYS).map_err(storage_error)?
...
fn flow_context_value(flow_id: &str, state: &State<'_, AppState>) -> Result<Value, AppError> {
    ...
    let request_body = compare_commands::body_for_compare(
        detail.request.as_ref().and_then(|request| request.body.as_ref()),
        state,
    )?;
    let response_body = compare_commands::body_for_compare(
        detail.response.as_ref().and_then(|response| response.body.as_ref()),
        state,
    )?;
...
fn preview_value(value: &Value, state: &State<'_, AppState>) -> Result<AiContextPreview, AppError> {
    build_context_preview(value, &context_policy(state)?)
        .map_err(ai_error)
}
...
async fn run_ai(
    ...
    if settings.provider != OPENAI_PROVIDER_ID {
        return Err(AppError::new("ai_provider_unsupported", "Configured AI provider is not available.", true));
    }
    ...
    let response = provider.generate(AiProviderRequest {
        task: task.clone(),
        context_json: preview.json.clone(),
    }).await.map_err(ai_error)?;
    ...
    state.ai_database.insert(&record).map_err(ai_storage_error)?;
    Ok(AiGenerationResult { record, context: preview })
}
...
fn normalize_secret_keys(values: Vec<String>) -> Vec<String> {
    ...
    output.sort();
    output.dedup();
    if output.is_empty() { default_secret_json_keys() } else { output }
}

fn storage_error(error: storage::StorageError) -> AppError { AppError::storage(error.to_string()) }
fn ai_storage_error(error: ai_storage::AiStorageError) -> AppError { AppError::storage(error.to_string()) }
fn secret_error(error: secret_store::SecretStoreError) -> AppError { AppError::new(error.code, error.message, true) }
fn ai_error(error: ai_core::AiError) -> AppError { AppError::new(error.code, error.message, error.recoverable) }
fn json_error(error: serde_json::Error) -> AppError { AppError::new("ai_settings_invalid", error.to_string(), true) }
